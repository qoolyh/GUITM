...
def GBTM():
    fileOBJ = open("data/" + cate + "/" + srcFolder + "_" + tarFolder + 'TTT.txt', 'w', encoding="utf-8")
...
def graph_match(prev_src_idx, prev_tar_idx, curr_src_idx, steps, prev_tar_elem_info):
    ...
    # at the internal
        ...
        for i in range(len(tar_array)):
            ...
            if e_match == -1:
                continue
            if "class" in src_elem and "EditText" in src_elem["class"]:
...
def elem_sim(tgt_elem, src_elem, prev_tgt_elem_info):
    ...
    if key in eSim:
        ...
        v = contentScore + idScore + descScore
        v /= 3
        eSim.update({key: v})
    return v
